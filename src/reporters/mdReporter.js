 ...
 * @returns {string}       GFM Markdown 문자열
 */
function toMarkdown(result) {
  ...
  const levelSection = levelStats ? [
    ...
    '| 항목 | 값 |',
    '|------|----|',
    '| 평균 | ' + levelStats.mean.toFixed(1) + ' |',
    '| 중앙값 | ' + levelStats.median.toFixed(1) + ' |',
    '| 최고 | ' + levelStats.max + ' |',
    '| p25 / p75 | ' + levelStats.p25.toFixed(1) + ' / ' + levelStats.p75.toFixed(1) + ' |',
  ].join('\n') : '';

  const lines = [
    '## ' + emoji + ' ' + name + ' — Fun Meter 리포트 (' + zone + ')',
    '',
    '> ' + runs + '회 실행 · ' + new Date().toLocaleString('ko-KR'),
    '',
    '## 생존 시간 통계',
    '',
    '| 항목 | 값 |',
